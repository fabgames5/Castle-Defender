...
public class _BuildSystem_Connection : MonoBehaviour
{
    ...
    public void pingPosition(_BuildSystem_TerrainPlacer tP)
    {
        ...
        hit = Physics.OverlapSphere(gameObject.transform.position, 1.0f, layerMask: ~(connectionLayer));
        if (hit.Length > 0)
        {
            ...
            for (int i = 0; i < hit.Length; i++)
            {
                ...
                {
                    if (hit[i] == parentBuilding.GetComponent<_BuildSystem_Construction>().spherePoints[j].GetComponent<Rigidbody>().GetComponent<Collider>())
                    {
                        cols.Remove(hit[i]);
            ...
            if (cols.Count > 0)
            {
                for (int i = 0; i < cols.Count; i++)
                {
                    ...
                    SnapPosition(cols[i].transform.position);
